...
def parse_args(give_args=None):
    """
    Parse command-line arguments for UTLC training and evaluation.
    Returns an argparse.Namespace with all options set.
    """
    parser = argparse.ArgumentParser(description="UTLC")
    # Hardware and reproducibility options
    ...
    parser.add_argument(
        "--mask_type",
        default="normal",
        choices=("uneven_1", "uneven_2", "normal", "raster"),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--lr_decay", type=int, default=25, help="learning rate decay per N epochs"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--load", type=str, default="none", choices=("none", "best", "last")
    )
    ...
    # Set epochs to a large value if 0 is given
    if args.epochs == 0:
        args.epochs = 1e8
    ...
    for arg in vars(args):
        if vars(args)[arg] == "True":
            ...
            vars(args)[arg] = False
    # Set CPU flag if CUDA is not available

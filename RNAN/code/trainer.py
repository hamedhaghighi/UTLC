...
class Trainer:
    """
    Trainer for RNAN model.
    Manages training/validation loops, optimizer, scheduler, and checkpointing.
    """
    ...
    def __init__(self, args, loader, checkpoint):
        ...
        # Stage weights for loss calculation
        if self.stages == 16 and args.mask_type.startswith("uneven"):
            if args.mask_type == "uneven_1":
                self.stage_weights = np.array(
                    [1, 1, 1, 1, 2, 2, 2, 2, 4, 4, 4, 4, 8, 8, 8, 16], dtype=np.float32
                )
            else:
                self.stage_weights = np.array(
                    [1, 1, 1, 1, 2, 2, 4, 4, 4, 4, 4, 4, 8, 8, 8, 8], dtype=np.float32
                )
        else:
        ...
        try:
            self.model, self.optimizer = amp.initialize(
                self.model, self.optimizer, opt_level="O1" if args.apex else "O0"
            )
        except:
            pass
        self.model = utility.place_model(self.model, args)
        self.ckp.write_log(
            "number of parameters in the enhancer: {}".format(
                utility.count_parameters(self.model)
            )
        )
    ...
    def load_all(self, mode):
        ...
        state = torch.load(
            os.path.join(self.ckp.dir, "model/{}.pt".format(mode)), **kwargs
        )
    ...
    def calc_loss(self, sr, hr, current_mask, next_mask):
        # Calculate masked loss for current stage
    ...
    def get_batch_stage_from_int(self, stage, B):
        # Create a batch tensor for the current stage
    ...
    def generate_mask(self, B, H, W, stage):
        # Generate current and next mask for the batch
    ...
    def get_stages_per_batch(self):
        # Return the number of stages per batch
        return self.stages
    ...
    def step_stage(
        ...
        # Perform a single stage of training or validation
    ...
    def step(self, is_train):
        # Main training or validation loop for one epoch
        ...
        train_batches = (
            200000
            if self.args.dataset.startswith("imagenet")
            else (100000 if self.args.dataset == "tiny" else 40000)
        )
        train_batches //= self.args.batch_size
        val_batches = (
            20000
            if self.args.dataset.startswith("imagenet")
            else (10000 if self.args.dataset == "tiny" else 10000)
        )
        ...
        with torch.set_grad_enabled(is_train):
            for batch in trange(
                train_batches if is_train else val_batches,
                desc="Train Step" if is_train else "Validation Step",
                dynamic_ncols=True,
            ):
                orig_lr, hr, bpg_size = next(
                    self.loader_train if is_train else loader_val
                )
                ...
                for stage in range(stages_per_batch):
                    ...
                    hidden_state, total_loss, recurrent_loss, loss_per_stage = (
                        packed_res
                    )
                ...
                if is_train and (batch + 1) % self.args.print_every == 0:
                    total_loss = (
                        total_loss / print_batch / stages_per_batch / conversion
                    ) + code_size.mean()
                    self.ckp.write_log(
                        "Train:\t{:.12f} in {:.2f}s".format(
                            total_loss, timer_model.release()
                        )
                    )
                    self.ckp.add_scalar(
                        "train_loss", total_loss, batch + (epoch - 1) * train_batches
                    )
                    total_loss = 0
                    print_batch = 0
                    code_size = np.array([])
        if not is_train:
            ...
            val_loss = (
                weighted_loss_per_stage.sum() / self.stage_weights.sum() + code_size
            )
            self.ckp.write_log(f"DetailedLoss: {loss_per_stage}")
            self.ckp.write_log(
                f"val_Loss: {val_loss}, non_normalized: {loss_per_stage.mean() + code_size}"
            )
            self.ckp.add_scalar("val_loss", val_loss, epoch * train_batches)
            for i in range(len(loss_per_stage)):
                self.ckp.add_scalar(
                    f"detailed_loss_{i}", loss_per_stage[i], epoch * train_batches
                )
    ...
    def terminate(self):
        # Check if training should terminate based on epoch count

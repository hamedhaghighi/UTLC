...
def process(mode, img_size, last_dataset_index=0, last_dataset_offset=0, is_train=False):
    ...
    if mode == 'test':
        with open(os.path.expanduser('~/.torch/data/imagenet_{}/val_data'.format(img_size)), 'rb') as f:
            dataset = pickle.load(f)['data'].reshape((-1, 3, img_size, img_size))
    else:
        with open(os.path.expanduser(
                '/hdd/Danial/Compression_Dataset/Imagenet64_train/train_data_batch_{}'.format(1 + last_dataset_index)), 'rb') as f:
            dataset = pickle.load(f)['data'][last_dataset_offset:].reshape((-1, 3, img_size, img_size))
    current_offset = 0
    for i in trange(expected_len, desc=mode, dynamic_ncols=True):
        ...
        if i - current_offset + 1 == len(dataset):
            ...
            with open(os.path.expanduser(
                    '/hdd/Danial/Compression_Dataset/Imagenet{}_train/train_data_batch_{}'.format(img_size, 1 + last_dataset_index)),
                    'rb') as f:
                dataset = pickle.load(f)['data'].reshape((-1, 3, img_size, img_size))
...
def main(img_size):
    ...
    if not os.path.exists(os.path.expanduser(path)):
        raise ValueError('please download and place imagenet_{} in {}'.format(img_size, path))
